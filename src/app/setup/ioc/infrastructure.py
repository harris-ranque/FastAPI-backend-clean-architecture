...
from app.infrastructure.auth.session.ports import transport
...
class MainAdaptersProvider(Provider):
    ...
    @provide
    def provide_hasher_semaphore(
        self,
        security: SecuritySettings
    ) -> HasherSemaphore:
        return HasherSemaphore(
            asyncio.Semaphore(security.password.hasher_max_threads)
        )
