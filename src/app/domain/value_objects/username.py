...
class Username(ValueObject):
    ...
    PATTERN_START: ClassVar[Final[re.Pattern[str]]] = re.compile(r"^[a-zA-Z0-9]")

    # - can contain multiple special characters . - _ between letters and digits
    PATTERN_ALLOWED_CHARS: ClassVar[Final[re.Pattern[str]]] = re.compile(r"[a-zA-Z0-9._-]",)
    ...
    PATTERN_END: ClassVar[Final[re.Pattern[str]]] = re.compile(r".*[a-zA-Z0-9]$",)
    ...
    def _validate_username_pattern(self, username_value: str) -> None:
        ...
        if not re.match(self.PATTERN_START, username_value):
            raise DomainTypeError(
                f"Username must start with a letter (A-Z, a-z) or a digit (0-9)."
            )
