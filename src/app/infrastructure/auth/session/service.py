...
class AuthSessionService:
    ...
    async def terminate_current_session(self) -> None:
        ...
        if self._cached_auth_session is not None:
            ...
            auth_session_id = await self._auth_session_transport.extract_id()
            if auth_session_id is None:
                log.warning(
                    "Terminate current session failed: partially failed. "
                    "Session ID can't be extracted from the transport. "
    ...
    async def terminate_all_sessions_for_user(self, user_id: UserId) -> None:
        ...
        log.debug("Terminate all sessions for user: started. User ID: '%s'.", user_id.value)
    ...
    async def _get_current_auth_session(self) -> AuthSession | None:
        """:raises AuthenticationError:"""
    ...
    async def _validate_and_extend_session(
        ...
        if (
            ...
            log.debug(
                "Validate and extend auth session: validate without extension. "
                "Auth session ID: '%s'.",
                auth_session.id_
            )
            return auth_session
